...
static void write_board(FILE *file,board brade)
{
	...
	for(row=0;row<8;row++)
	{
		...
		{
			char c='.';
			if(GETBIT(brade.black,8*row+col))
			{
				c='x';
			}
			else if(GETBIT(brade.white,8*row+col))
			{
				c='o';
			}
			fputc(c,file);
...
bool save_position(char *filename,position_t *pos)
{
	for(;;)
	{
		...
		if(file!=NULL)
		{
			...
			if(strlen(pos->computername))
			{
				fprintf(file,"computername %s\n",pos->computername);
			}
			if(strlen(pos->humanname))
			{
				fprintf(file,"humanname %s\n",pos->humanname);
			}
			fprintf(file,"moves ");
			for(i=0;i<pos->nummoves;i++)
			{
				...
				{
					fprintf(file,"%c%c ",
							pos->moves[i]%8+'a',pos->moves[i]/8+'1');
				}
				else
				{
					fprintf(file,"ps ");
		...
		{
			fprintf(stderr,"errno %d opening file %s\n",errno,filename);
		}
		filename=0;
	}
}
	...
	for(;;)
	{
		...
		if(file!=NULL)
		{
			for(;;)
			{
				...
				{
					if(strncmp(buf+14,"black",5)==0)
					{
						pos->computer=black;
					}
					else if(strncmp(buf+14,"white",5)==0)
					{
						pos->computer=white;
					}
					else
				...
				{
					if(strncmp(buf+11,"black",5)==0)
					{
						pos->turntomove=black;
					}
					else if(strncmp(buf+11,"white",5)==0)
					{
						pos->turntomove=white;
					}
					else
					{
				...
				{
					if(strcmp(buf+14,"true")==0)
					{
						pos->deterministic=true;
					}
					else if(strcmp(buf+14,"false")==0)
					{
						pos->deterministic=false;
					}
					else
